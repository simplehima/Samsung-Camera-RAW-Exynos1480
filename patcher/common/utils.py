...
def create_magisk_module(
        ...
        module_version: str, description: str
    ):
    if len(libs) == 0:
    ...
    if not os.path.isdir(module_base_dir):
        # We don't have ModuleBase, so we skip module creation and warn
        warn('ModuleBase not found, skipping Magisk module creation.')
        return
